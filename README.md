...
**Вариант «всё в одном» (рекомендуется для нового проекта):**

1. В левом меню Supabase нажми **«SQL Editor»** → **«+ New query»**.
2. Открой в проекте файл **`db/full_schema.sql`**, скопируй весь текст (Ctrl+A, Ctrl+C).
3. Вставь код в окно SQL Editor и нажми **«Run»**.
4. Должно появиться **«Success. No rows returned»** — таблицы, RLS и триггеры созданы одним скриптом.

**Итог:** база готова. Дальше нужно только создать бакеты Storage и первого учителя (см. ниже и шаг 1.4).

**Альтернатива (по частям):** можно по‑старому выполнить сначала `db/schema.sql`, затем по очереди файлы из `db/migrations/` (002, 003, 004, 005, 006, 007, 008 и т.д.). Для нового проекта проще один раз запустить `db/full_schema.sql`.

**Хранилище для фото (аватар):** в Supabase открой **Storage** → **New bucket** → имя `avatars`, включи **Public**. Политики RLS для этого бакета уже есть в `full_schema.sql`. Без создания бакета загрузка аватара выдаст ошибку.

**Хранилище для фото заданий:** **Storage** → **New bucket** → имя `assignment-images`, включи **Public**. Политики RLS для него тоже в `full_schema.sql`.

---

### Полная пересборка базы (сброс и заново)

Если нужно **стереть** все таблицы и данные в текущем проекте и написать базу заново (URL и ключи проекта не меняются):

1. В **SQL Editor** выполни скрипт **`db/reset_public.sql`** — удалятся таблицы, функции и типы в схеме `public`. Пользователи в **Authentication** не удаляются.
2. Затем выполни **`db/full_schema.sql`** — база создаётся заново.
3. Создай бакеты **avatars** и **assignment-images** (если удалял), создай первого учителя в Authentication и выполни `INSERT INTO profiles (id, role) VALUES ('UUID_УЧИТЕЛЯ', 'teacher') ON CONFLICT (id) DO UPDATE SET role = 'teacher';`.

**Новый проект Supabase:** если создаёшь новый проект с нуля, достаточно один раз выполнить `db/full_schema.sql`, затем бакеты и первого учителя.
...
| Создать таблицы | Supabase → SQL Editor | Вставить `db/full_schema.sql` → Run (или `db/schema.sql` + миграции по порядку) |
